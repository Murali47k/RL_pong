"""
main.py — Pong simulation, both players move randomly. No PyTorch, no RL.
"""
...
os.environ.setdefault("SDL_VIDEODRIVER", "x11")   # X11, not Wayland
...
class Slider:
    ...
    def draw(self, surf, fsm, fval):
        surf.blit(fsm.render(self.label, True, GRAY), (self.tx, self.ty - 26))
        pygame.draw.rect(surf, (70, 70, 80),
                         (self.tx, self.ty - 2, self.tw, 5), border_radius=3)
        kx = self._kx()
        if kx > self.tx:
            pygame.draw.rect(surf, GREEN,
                             (self.tx, self.ty - 2, kx - self.tx, 5), border_radius=3)
        pygame.draw.circle(surf, WHITE, (kx, self.ty), 8)
        vs = str(self.val) if self.dtype == int else f"{self.val:.2f}"
        surf.blit(fval.render(vs, True, YELLOW), (self.tx + self.tw + 6, self.ty - 8))
...
class Paddle:
    def __init__(self, x: int, color, spd: int = PADDLE_SPD):
        self.rect  = pygame.Rect(x, GAME_H // 2 - PADDLE_H // 2, PADDLE_W, PADDLE_H)
        self.color = color
        self.spd   = spd
        self._action     = 1          # 1=UP, 0=DOWN
        self._steps_left = 0          # frames remaining for current action

    def random_step(self):
        """Pick a new random direction every few frames."""
    ...
    def draw(self, surf, ox: int):
        pygame.draw.rect(surf, self.color, self.rect.move(ox, 0), border_radius=3)
...
class Ball:
    ...
    def reset(self):
        self.rect    = pygame.Rect(GAME_W // 2, GAME_H // 2, BALL_SZ, BALL_SZ)
...
class App:
    def __init__(self):
        ...
        self.sliders = {
            "num_games":  Slider("# Games",       10,  500, 20,  int,  10),
        ...
        # per-game stats
        self.rallies:   list[int]   = []   # hits per game
        self.durations: list[float] = []   # seconds per game
        self._rally_count = 0
    ...
    def _draw_sidebar(self):
        ...
        if self.state in (RUNNING, SUMMARY):
            y0 = self.btn.top - 110
            pygame.draw.line(sb, (60, 60, 70), (8, y0 - 8), (SIDEBAR_W - 8, y0 - 8))
            rows = [
                (f"Game  {self.game_num}/{self.num_games}", WHITE),
            ...
            for i, (txt, col) in enumerate(rows):
                sb.blit(self.f_sm.render(txt, True, col), (14, y0 + i * 20))

        # button
        if self.state == SETUP:
            hov = self.btn.collidepoint(pygame.mouse.get_pos())
            pygame.draw.rect(sb, (70, 210, 110) if hov else (45, 150, 75),
                             self.btn, border_radius=8)
            bt = self.f_title.render("▶  Start Simulation", True, WHITE)
        elif self.state == RUNNING:
            pygame.draw.rect(sb, (60, 60, 70), self.btn, border_radius=8)
            bt = self.f_sm.render("Simulating…", True, GRAY)
        else:
            pygame.draw.rect(sb, (50, 80, 160), self.btn, border_radius=8)
            bt = self.f_title.render("▶  Run Again", True, WHITE)

        sb.blit(bt, (self.btn.centerx - bt.get_width() // 2,
    ...
    def _draw_game(self):
        ...
        for y in range(0, GAME_H, 20):
            pygame.draw.rect(surf, (45, 45, 45), (ox + GAME_W // 2 - 1, y, 2, 12))

        if self.state == SETUP:
            msg = self.f_big.render("Configure & press  Start →", True, (70, 70, 82))
            surf.blit(msg, (ox + GAME_W // 2 - msg.get_width() // 2,
        ...
        sc = self.f_hud.render(f"{self.score_l}  :  {self.score_r}", True, WHITE)
        surf.blit(sc, (ox + GAME_W // 2 - sc.get_width() // 2, 10))
        gc = self.f_sub.render(f"Game {self.game_num} / {self.num_games}", True, GRAY)
        surf.blit(gc, (ox + GAME_W // 2 - gc.get_width() // 2, 44))
        surf.blit(self.f_sub.render("P1", True, RED),  (ox + 24, 10))
        surf.blit(self.f_sub.render("P2", True, BLUE), (ox + GAME_W - 40, 10))

        # rally counter bottom-centre
        rc = self.f_sm.render(f"rally hits: {self._rally_count}", True, (100, 100, 110))
        surf.blit(rc, (ox + GAME_W // 2 - rc.get_width() // 2, GAME_H - 22))
    ...
    def _draw_summary(self, ox: int):
        ...
        for txt, col, font in rows:
            ...
            surf.blit(s, (ox + GAME_W // 2 - s.get_width() // 2, y))
            y += font.get_height() + 8
    ...
    def _sim_step(self):
        b, pl, pr = self.ball, self.paddle_l, self.paddle_r

        pl.random_step()
        pr.random_step()
        b.move()

        # collision — push ball out to avoid sticking
        ...
        if self.score_l >= MAX_PTS or self.score_r >= MAX_PTS:
            ...
            if self.score_l > self.score_r:
                ...
                self.total_wins[1] += 1
            print(f"[{self.game_num}/{self.num_games}] "
                  f"P1 {self.score_l}-{self.score_r} P2  "
    ...
    def run(self):
        while True:
            self.clock.tick(self.fps_val if self.state == RUNNING else 60)
